	...
	<view>
		...
		<scroll-view :scroll-y="true" class="scroll-view-body">
			<view class="padding-bottom150">
				<!-- :flag="8" -->
				<fromDeatil :msg="showMsg(item.mastertype)" :item="item" v-for="(item, index) in list" :key="index" @getDetail="getDetail">
					<view class="slot-warp" v-if="audit === 'noThrough'">
						<view class="slot-active" @click="_start(item.order_id, TYPES.REVIEW)">
							完成
						</view>
					</view>
				</fromDeatil>
...
<script>
	...
	import {
		showMsg
	} from "@/utils/showMsg.js";
	import {
		start
	} from "@/components/api/api.js";
	import {
		workersOrderCenterAllStatus
	...
	export default {
		...
		methods: {
			...
			getDetail(item) {
				...
				uni.navigateTo({
					url: "../../components/workersOrderDetail/allDetail?info=" +
						JSON.stringify(item),
				});
			},
		},
