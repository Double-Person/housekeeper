<template>
	...
	<scroll-view :scroll-y="true" class="scroll-view-tab-list-body">
		...
		<view class="padding-bottom150">
			<!-- :flag="8" -->
			<fromDeatil msg="msg" :item="item" v-for="(item, index) in list" :key="index" @getDetail="getDetail()">
				<view class="slot-warp" v-if="audit == 'review'">
					<view class="slot-active" @click="starts(item.order_id, TYPES.START)">开工</view>
				</view>
				
				<view class="slot-warp" v-if="audit == 'haveBeenThrough'">
					<view class="slot-not-active" @click="starts(item.order_id, TYPES.CONSTRUCTION)">添加进度</view>
